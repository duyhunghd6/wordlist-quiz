...
const SAMPLE_SENTENCES = [
  { before: 'I', after: 'to the park yesterday.', options: ['go', 'went', 'gone', 'going'], correct: 'went' },
  { before: 'She', after: 'her homework every day.', options: ['do', 'does', 'did', 'doing'], correct: 'does' },
  { before: 'They', after: 'playing football right now.', options: ['is', 'are', 'was', 'were'], correct: 'are' },
  { before: 'He', after: 'a book last night.', options: ['read', 'reads', 'reading', 'readed'], correct: 'read' },
  { before: 'We', after: 'to school by bus.', options: ['go', 'goes', 'went', 'going'], correct: 'go' },
  { before: 'The cat', after: 'on the mat.', options: ['sit', 'sits', 'sat', 'sitting'], correct: 'sits' },
  { before: 'I', after: 'already eaten lunch.', options: ['have', 'has', 'had', 'having'], correct: 'have' },
  { before: 'She', after: 'to music every morning.', options: ['listen', 'listens', 'listened', 'listening'], correct: 'listens' },
];

const AngryTensesGame = ({ words, tenseSentences, onAnswer, onComplete, onHome }) => {
  ...
  const initPhysics = (questionData) => {
    ...
    setTimeout(() => {
      ...
      let currentBird = Bodies.circle(slingX, slingY, 60, {
        restitution: 0.5, density: 0.004, label: 'bird',
        render: { opacity: 0 },
      });
      ...
      const targets = opts.map((word, i) => {
        ...
        return Bodies.circle(tx, ty, pigSize / 2, {
          restitution: 0.3, density: 0.004, friction: 1.0, frictionStatic: 0.8,
          label: `pig-${i}`,
          render: { opacity: 0 },
          isTarget: true, isCorrectTarget, text: word,
        });
      });

      // Wooden planks (static structure that holds pigs)
      const plankStyle = { fillStyle: '#D2A679', strokeStyle: '#6b3e12', lineWidth: 2 };
      const floorStyle = { fillStyle: '#CD853F', strokeStyle: '#6b3e12', lineWidth: 2 };
      const planks = [
        Bodies.rectangle(targetX - 35, groundY - 20, 12, 60, {
          isStatic: true, render: plankStyle, label: 'plank-0',
        }),
        Bodies.rectangle(targetX + 35, groundY - 20, 12, 60, {
          isStatic: true, render: plankStyle, label: 'plank-1',
        }),
        Bodies.rectangle(targetX, groundY - 55, 90, 10, {
          isStatic: true, render: floorStyle, label: 'floor-0',
        }),
        Bodies.rectangle(targetX - 15, groundY - 90, 12, 50, {
          isStatic: true, render: plankStyle, label: 'plank-2',
        }),
        Bodies.rectangle(targetX + 15, groundY - 90, 12, 50, {
          isStatic: true, render: plankStyle, label: 'plank-3',
        }),
        Bodies.rectangle(targetX, groundY - 120, 80, 10, {
          isStatic: true, render: floorStyle, label: 'floor-1',
        }),
      ];

      Composite.add(world, [ground, currentBird, elastic, ...targets, ...planks]);
      ...
      const pigEls = targets.map((t) => {
        ...
        return {
          el: createSprite('at-pig-sprite', pigSize,
      ...
      Events.on(engine, 'collisionStart', (event) => {
        ...
        for (const pair of event.pairs) {
          ...
          if (target && !target.isDestroyed && Math.max(a.speed, b.speed) > 3) {
            ...
            setTimeout(() => {
              try { Composite.remove(world, target); } catch (e) { /* */ }
            }, 300);
    ...
    return (
      <div className="at-wrapper">
        <div className="at-end-screen">
          ...
          <h2 className="at-end-score">Score: <span>{score}</span></h2>
          <p className="at-end-sub">{correctCount} / {questions.length} correct</p>
  ...
  return (
    <div className="at-wrapper">
      {/* Background */}
      <div className="at-sky"><div className="at-clouds"></div></div>
      <div className="at-ground"></div>

      {/* HUD */}
      <div className="at-hud">
        <button className="at-hud-btn" onClick={onHome}><Home size={20} /></button>
        <div className="at-hud-score">⭐ {score}</div>
        <div className="at-hud-progress">{currentIndex + 1}/{questions.length}</div>
        {phase === 'physics' && <div className="at-hud-timer">⏱️ {timeLeft}s</div>}
      </div>
        ...
        <div className="at-grammar-overlay">
          <div className="at-question-box">
            ...
            <div className="at-sentence-display">
              ...
              <div className="at-scroll-picker" onClick={handleDoubleTap}>
                  ...
                  <div className="at-picker-track">
                    {currentQ.options.map((opt, i) => {
                      ...
                      if (confirmed && i === activeOption) {
                        cls += isCorrectAnswer ? ' at-picker-correct' : ' at-picker-wrong';
                      }
                      return (
                        <div
                          key={opt}
                          className={cls}
                          onClick={(e) => { e.stopPropagation(); setActiveOption(i); }}
                        >
                          {opt}
                        </div>
                      );
            ...
            <div className="at-picker-instructions">
              Scroll, drag, or tap. <br/> <b>Double-Tap</b> or <b>Enter</b> to confirm.
            </div>
          </div>
        </div>
      )}
      ...
      {phase === 'physics' && (
        <div className="at-physics-phase">
          <div className="at-sentence-board">
            {currentQ.before} <span className="at-correct-word">{currentQ.correct}</span> {currentQ.after}
          </div>
