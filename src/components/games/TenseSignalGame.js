...
const TenseSignalGame = ({ words, numQuestions = 10, isAllQuestions = false, onAnswer, onComplete, onHome }) => {
  ...
  const startWorld = useCallback((world) => {
    ...
    if (world.id === 'mix_all') {
      ...
      const count = isAllQuestions ? world.questions.length : Math.min(numQuestions || words?.length || 10, world.questions.length);
